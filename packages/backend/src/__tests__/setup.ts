...
vi.mock('../services/maps/mapbox-client.js', () => ({
  geocodingClient: {
    forwardGeocode: vi.fn().mockReturnValue({
      send: vi.fn().mockResolvedValue({
        body: {
          features: [{
            center: [151.2093, -33.8688],
            place_name: '123 Main St, Guildford NSW 2161, Australia',
          }],
        },
      }),
    }),
...
process.env.SESSION_SECRET = process.env.SESSION_SECRET || 'test-session-secret-minimum-64-chars-for-secure-sessions-DO-NOT-USE';
process.env.ENCRYPTION_KEY = process.env.ENCRYPTION_KEY || 'dGVzdC1lbmNyeXB0aW9uLWtleS0zMi1ieXRlcy1iYXNlNjQ=';
